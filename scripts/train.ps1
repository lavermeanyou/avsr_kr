# Training: thin wrapper around `python -m avsr.train` (see README.md).
#   .\scripts\train.ps1                                   # configs/base.yaml, resumes from last.pt if present
#   .\scripts\train.ps1 --set train.epochs=1 --set data.num_workers=2 --limit 64    # smoke test
# All arguments are passed through unchanged. The module runs from the project root, so relative paths are resolved
# against the project root. Quote values containing commas, e.g. --set "split.test_speakers=[E014,C159]".
